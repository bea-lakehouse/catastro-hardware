...
parsed as (
  select
    ...
    nullif(trim(coalesce(
      row_data->>'requiere_equipo',
      row_data->>'Requiere equipo',
      row_data->>'Requiere Equipo',
      row_data->>'solicitud_equipo_explicita',
      row_data->>'Solicitud equipo explicita',
      row_data->>'Solicitud equipo explícita',
      row_data->>'Solicitud de equipo',
      row_data->>'Equipo requerido',
      row_data->>'Requiere computador',
      row_data->>'Necesita equipo'
    )), '') as solicitud_equipo_explicita_raw,
    nullif(trim(coalesce(
      row_data->>'Location ingreso',
      row_data->>'Location Ingreso',
      row_data->>'Location',
...
normalized as (
  select
    ...
    cliente,
    case
      when solicitud_equipo_explicita_raw is null then null
      when upper(trim(solicitud_equipo_explicita_raw)) in ('SI', 'SÍ', 'YES', 'TRUE', '1', 'X', 'REQUERIDO', 'SOLICITADO') then true
      when upper(trim(solicitud_equipo_explicita_raw)) in ('NO', 'FALSE', '0', 'N/A', 'NA', 'NO REQUIERE', 'REMOTO', 'SIN EQUIPO', 'NO APLICA') then false
      else null
    end as solicitud_equipo_explicita,
    pais_ingreso,
    perfil,
    marca,
    modelo,
